            ...
            <div class="bg-white rounded-3xl shadow-2xl p-8 mb-8">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <span class="error-message text-red-500 text-sm hidden"></span>
                        <p id="dniLookupMsg" class="text-xs text-gray-500 mt-1"></p>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" id="nombres" name="nombres"
                            class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-300"
                            placeholder="Ingresa tus nombres" disabled>
                        <span class="error-message text-red-500 text-sm hidden"></span>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" id="apellidos" name="apellidos"
                            class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-300"
                            placeholder="Ingresa tus apellidos" disabled>
                        <span class="error-message text-red-500 text-sm hidden"></span>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" id="fecha_nacimiento" name="fecha_nacimiento"
                            class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all duration-300" disabled>
                        <span class="error-message text-red-500 text-sm hidden"></span>
                    </div>
...
<script>
    ...
    // Mostrar un paso específico y exponerlo globalmente
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const dniMsg = document.getElementById('dniLookupMsg');

        const clearDniFields = () => {
        ...
        const consultarDni = async () => {
            ...
            try {
                if (dniMsg) dniMsg.textContent = 'Consultando DNI...';

                const res = await fetch(`/api-publico/dni/complete/${encodeURIComponent(dni)}`, {
                    headers: {
                        'Accept': 'application/json'
                    }
                });
                ...
                if (!res.ok || !json.ok) {
                    clearDniFields();
                    if (dniMsg) dniMsg.textContent = json.message || 'No se pudo consultar el DNI.';
                    return;
                }

                if (nombresInput) nombresInput.value = json.nombres || '';
                if (apellidosInput) apellidosInput.value = json.apellidos || '';
                if (fechaNacInput) fechaNacInput.value = json.fecha_nacimiento || '';
                if (edadInput) edadInput.value = (json.edad ?? '') + '';

                if (dniMsg) dniMsg.textContent = '';
            } catch (e) {
                clearDniFields();
                if (dniMsg) dniMsg.textContent = 'Error consultando el DNI.';
            }
        };

        // ✅ Recomendado: blur (y opcional input con debounce)
        if (dniInput) {
            const consultarDniDebounced = debounce(consultarDni, 600);
            dniInput.addEventListener('input', consultarDniDebounced);
