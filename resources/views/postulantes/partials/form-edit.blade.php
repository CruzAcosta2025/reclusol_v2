<div class="relative p-6 bg-white rounded-lg">
    ...
    @php
        ...
        $fechaP = old('fecha_postula', optional(\Illuminate\Support\Carbon::parse($postulante->fecha_postula))->format('Y-m-d'));
        $fechaN = old('fecha_nacimiento', optional(\Illuminate\Support\Carbon::parse($postulante->fecha_nacimiento))->format('Y-m-d'));
        ...
        $esOperativo = in_array($tipoPersonalCodigo, ['01', '03'], true);
    @endphp
    ...
    <form id="form-edit" method="POST" action="{{ route('postulantes.update', $postulante) }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="fecha_postula_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
                    <i class="fas fa-calendar-alt text-blue-600"></i> Fecha de postulacion
                </label>
                <input type="date" id="fecha_postula_edit" name="fecha_postula" value="{{ $fechaP }}" required readonly
                    class="mt-1 block w-full border border-gray-400 text-gray-900 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-gray-100 cursor-not-allowed">
            ...
            {{-- Fecha de nacimiento --}}
            <div>
                <label for="fecha_nacimiento_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
            ...
            {{-- Nacionalidad --}}
            <div>
                <label for="nacionalidad_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="departamento_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
            ...
            <div>
                ...
                <select id="provincia_edit" disabled
                    ...
                    @foreach($provincias->where('DEPA_CODIGO', $depCod) as $codigo => $prov)
                        <option value="{{ $codigo }}" @selected($provCod === $codigo)>{{ $prov->PROVI_DESCRIPCION }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="distrito_edit" disabled
                    ...
                    @foreach($distritos->where('PROVI_CODIGO', $provCod) as $codigo => $dist)
                        <option value="{{ $codigo }}" @selected($distCod === $codigo)>{{ $dist->DIST_DESCRIPCION }}</option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            {{-- Tipo de cargo --}}
            <div>
                <label for="tipo_cargo_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
            ...
            <div>
                <label for="cargo_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
                    <i class="fas fa-clipboard-list text-blue-600"></i> Cargo solicitado
                </label>
                <select id="cargo_edit" name="cargo"
                    class="mt-1 block w-full border border-gray-400 text-gray-900 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    data-value="{{ $cargoCod }}">
                    <option value="">Selecciona el cargo</option>
                    @foreach($cargos as $codigo => $cargo)
                        <option value="{{ $codigo }}"
                            data-tipo="{{ str_pad((string) ($cargo->TIPO_CARG ?? ''), 2, '0', STR_PAD_LEFT) }}"
                            @selected($cargoCod === $codigo)>
                            {{ $cargo->DESC_CARGO ?? $cargo }}
                        </option>
                    @endforeach
                </select>
            ...
            @endphp
            <div>
                <label for="experiencia_rubro_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
            ...
            @endphp
            <div>
                <label for="grado_instruccion_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
            ...
            <div class="{{ $esOperativo ? '' : 'hidden' }}" data-operativo>
                <label for="sucamec_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
                    <i class="fas fa-id-badge text-blue-600"></i> SUCAMEC
                </label>
            ...
            <div class="{{ $esOperativo ? '' : 'hidden' }}" data-operativo>
                <label for="carne_sucamec_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
                    <i class="fas fa-address-card text-blue-600"></i> Carne SUCAMEC
                </label>
                ...
                @error('carne_sucamec') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div class="{{ $esOperativo ? '' : 'hidden' }}" data-operativo>
                <label for="licencia_arma_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
            ...
            <div class="{{ $esOperativo ? '' : 'hidden' }}" data-operativo>
                <label for="licencia_conducir_edit" class="block text-sm font-semibold text-gray-900 flex items-center gap-2">
            ...
            @unless($esOperativo)
                ...
                <input type="hidden" name="carne_sucamec" value="{{ old('carne_sucamec',$postulante->carne_sucamec ?? 'NO') }}">
                <input type="hidden" name="licencia_arma" value="{{ old('licencia_arma',$postulante->licencia_arma ?? 'NO') }}">
                <input type="hidden" name="licencia_conducir" value="{{ old('licencia_conducir',$postulante->licencia_conducir ?? 'NO') }}">
            @endunless
        </div>
