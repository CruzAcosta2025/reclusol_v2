...
<div class="relative p-6 bg-white rounded-lg">
    ...
    <form id="form-edit" method="POST" action="{{ route('postulantes.update', $postulante) }}" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="border-b pb-6 space-y-4">
            <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                <i class="fas fa-user text-blue-500"></i> Datos personales
            </h3>
        ...
        <div class="border-b pb-6 space-y-4">
            <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                <i class="fas fa-map-marker-alt text-blue-500"></i> Ubicacion y contacto
            </h3>
        ...
        <div class="border-b pb-6 space-y-4">
            <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                <i class="fas fa-briefcase text-blue-500"></i> Cargo
            </h3>
        ...
        <div class="border-b pb-6 space-y-4">
            ...
            {{-- Solo operativo --}}
            @php $operativoDisabled = $esOperativo ? '' : 'disabled'; @endphp
            <div id="campos-operativo-edit" data-operativo="{{ $esOperativo ? '1' : '0' }}"
                class="mt-6 pt-6 border-t {{ $esOperativo ? '' : 'hidden' }}">
                <h4 class="text-md font-semibold text-gray-700 mb-4">Requisitos Especiales (Operativo)</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="sucamec_edit" class="block text-sm font-medium text-gray-700">
                            Curso SUCAMEC
                        </label>
                        <select id="sucamec_edit" name="sucamec" {{ $operativoDisabled }}
                    ...
                    <div>
                        <label for="carne_sucamec_edit" class="block text-sm font-medium text-gray-700">
                            Carne SUCAMEC
                        </label>
                        <select id="carne_sucamec_edit" name="carne_sucamec" {{ $operativoDisabled }}
                            class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div>
                        ...
                        <select id="licencia_arma_edit" name="licencia_arma" {{ $operativoDisabled }}
                            class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div>
                        ...
                        <select id="licencia_conducir_edit" name="licencia_conducir" {{ $operativoDisabled }}
                            class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const tipoSel = form.querySelector('#tipo_cargo_edit');
        const camposOperativoEl = form.querySelector('#campos-operativo-edit');
        ...
        const pad2 = (val) => (val || '').toString().replace(/\D+/g, '').padStart(2, '0');
        const toggleOperativo = () => {
            if (!tipoSel || !camposOperativoEl) return;
            const show = pad2(tipoSel.value) === '02';
        ...
        toggleOperativo();
        if (tipoSel) {
            tipoSel.addEventListener('change', toggleOperativo);
        }
    });
</script>
