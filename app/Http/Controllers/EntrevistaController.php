...
class EntrevistaController extends Controller
{
    ...
    public function enviarWhatsapp(
        ...
        $data = $request->validate([
            'tipo'        => ['required', Rule::in(['apto', 'no_apto'])],
            'hora_inicio' => ['nullable', 'string', 'max:50'],
            'hora_fin'    => ['nullable', 'string', 'max:50'],
            'enlace_meet' => ['nullable', 'string', 'max:255'],
        ]);

        $numero = preg_replace('/\D+/', '', (string)($postulante->celular ?? ''));

        if (empty($numero)) {
            return response()->json([
                'success' => false,
                'message' => 'El postulante no tiene un número de celular registrado.',
            ], 422);
        }

        $postulante->loadMissing([
            'requerimiento',
            'entrevistas' => function ($q) {
                $q->latest('fecha_entrevista');
            },
        ]);

        $entrevista = $postulante->entrevistas->first();

        $fechaEntrevista = $entrevista?->fecha_entrevista instanceof Carbon
            ? $entrevista->fecha_entrevista
            : ($entrevista?->fecha_entrevista ? Carbon::parse($entrevista->fecha_entrevista) : null);

        $fecha      = $fechaEntrevista?->format('d/m/Y');
        $horaInicio = $data['hora_inicio'] ?: ($fechaEntrevista ? $fechaEntrevista->format('H:i') : null);
        $horaFin    = $data['hora_fin'] ?: ($fechaEntrevista ? $fechaEntrevista->copy()->addMinutes(20)->format('H:i') : null);

        $nombre = trim(($postulante->nombres ?? '') . ' ' . ($postulante->apellidos ?? ''));
        $puesto = $this->obtenerPuestoPostulante($postulante);

        $mensaje = $this->construirMensajeWhatsapp(
            $data['tipo'],
            $nombre,
            $puesto,
            $fecha,
            $horaInicio,
            $horaFin,
            $data['enlace_meet'] ?? null
        );

        try {
            $apiResponse = $whatsappService->sendMessage($numero, $mensaje);
        } catch (\Throwable $e) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'No se pudo enviar el mensaje de WhatsApp. Revisa la configuración o intenta más tarde.',
            ], 500);
        }
    ...
    private function obtenerPuestoPostulante(Postulante $postulante, $cargos = null): string
    {
        $postulante->loadMissing('requerimiento');
        $cargos = $cargos ?? Cargo::forSelect();

        if ($postulante->requerimiento) {
            $codCargo = str_pad((string)$postulante->requerimiento->cargo_solicitado, 4, '0', STR_PAD_LEFT);
            return $cargos->get($codCargo) ?? $postulante->requerimiento->cargo_solicitado ?? 'N/A';
        }

        if (!empty($postulante->cargo)) {
            $codCargo = str_pad((string)$postulante->cargo, 4, '0', STR_PAD_LEFT);
            return $cargos->get($codCargo) ?? $postulante->cargo;
        }

        return 'el puesto indicado';
    }

    private function construirMensajeWhatsapp(
        string $tipo,
        string $nombre,
        string $puesto,
        ?string $fecha,
        ...
        $puesto = trim($puesto) !== '' ? $puesto : 'el puesto indicado';

        if ($tipo === 'apto') {
            $segmentoFecha = $fecha ? " para el día {$fecha}" : '';
            $segmentoHora  = ($horaInicio && $horaFin)
                ? " entre las {$horaInicio} y las {$horaFin}"
                : '';

            $mensaje = "Hola {$nombre}, gracias por postular al puesto de {$puesto}. Luego de realizar la revisión documentaria correspondiente, solicitamos de tu tiempo para realizar una entrevista virtual mediante Google Meets cuya duración será de 20 minutos{$segmentoFecha}{$segmentoHora}. Indícanos si estás disponible en ese horario o si deseas reprogramar tu entrevista.";

            if (!empty($enlace)) {
                $mensaje .= " Enlace de la reunión: {$enlace}.";
            }
